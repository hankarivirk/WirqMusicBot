...
from anony import config, spawn_task
...
class Telegram:
    ...
    async def download(self, msg: types.Message, sent: types.Message) -> Media | None:
        ...
        file_ext = getattr(media, "file_name", "") or ""
        file_ext = file_ext.rsplit(".", 1)[-1] if "." in file_ext else ""
        if not file_ext:
            # No filename (voice notes never have one; some audio/video
            # messages don't either) — fall back to a sensible
            # extension based on the actual media type instead of
            # leaving this empty, which produced a path like
            # "downloads/<id>." with a trailing dot and no extension.
        ...
        if file_size > config.MAX_FILE_SIZE:
            await sent.edit_text(sent.lang["dl_limit"].format(config.MAX_FILE_SIZE // (1024 * 1024)))
            return await sent.stop_propagation()
        ...
        try:
            ...
            if not os.path.exists(file_path):
                ...
                task = spawn_task(
                    ...
                    name=f"telegram-download:{msg_id}",
                )
