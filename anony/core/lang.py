...
class Language:
    ...
    async def get_lang(self, chat_id: int) -> dict:
        lang_code = await anony.db.get_lang(chat_id)
    ...
    def language(self):
        def decorator(func):
            ...
            async def wrapper(*args, **kwargs):
                ...
                if anony.db is None:
                    logger.error("Language middleware invoked before database initialization")
                    return

                if chat.id in anony.db.blacklisted:
                    logger.info(f"Chat {chat.id} is blacklisted, leaving...")
                    return await chat.leave()

                lang_code = await anony.db.get_lang(chat.id)
                if lang_code not in self.languages:
                    logger.warning(
