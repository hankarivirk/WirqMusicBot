...
async def _broadcast(_, message: types.Message):
    ...
    async with broadcasting:
        for chat in chats:
            # Retry once after a FloodWait instead of moving on and losing
            # the chat entirely (previously it wasn't retried, logged, or
            # counted anywhere — success or failure).
            for attempt in range(2):
                try:
                    ...
                    failed.write(f"{chat} - FloodWait after retry ({fw.value}s)\n")
                    break
