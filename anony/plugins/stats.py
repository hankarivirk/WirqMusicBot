...
async def _stats(_, m: types.Message):
    ...
    if m.from_user.id in app.sudoers:
        ...
        # process.cpu_percent(interval=1.0) is a blocking synchronous call
        # that would otherwise freeze the whole event loop for ~1s on
        # every /stats. Run it in a worker thread instead.
        cpu_percent = await asyncio.to_thread(process.cpu_percent, 1.0)
