...
async def _seek(_, m: types.Message):
    ...
    if m.command[0] == "seekback":
        ...
        start_from = media.time + to_seek
        if start_from >= media.duration_sec - 1:
            # Seeking forward past (or right up to) the end doesn't
            # leave anywhere sensible to land — for a short enough
            # track this could even go negative. Skip to the next
            # track instead, which is what "seek past what's left"
            # actually means in practice.
            try:
                await sent.delete()
            except Exception:
                pass
            return await anon.play_next(m.chat.id)
