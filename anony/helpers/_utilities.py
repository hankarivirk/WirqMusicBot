...
class Utilities:
    ...
    def format_duration(self, seconds: int) -> str:
        """M:SS for anything under an hour, H:MM:SS past that — plain
        track-duration style (no unit suffix), unlike format_eta above.
        `time.strftime("%M:%S", time.gmtime(seconds))` — used in a
        couple of places before this existed — silently wraps at 60
        minutes with no hour shown at all, so a 1:05:20 track displayed
        as just "05:20". This does the same simple MM:SS for anything
        under an hour, but correctly carries the hour past that."""
