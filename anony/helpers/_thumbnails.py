...
class Thumbnail:
    ...
    async def generate(self, song: Track, size=None) -> str:
        ...
        async with lock:
            try:
                ...
                output = f"cache/{song.id}_{self._brand_tag()}.jpg"
                if os.path.exists(output):
                    return output

                await self.save_thumb(temp, song.thumbnail)
                # Everything past this point is pure CPU-bound PIL work
                # (blur passes, compositing, font rendering, no further
                # awaits) — offloaded to a thread so it doesn't freeze
                # the single shared event loop (every other chat's
                # commands/callbacks, not just this one) for the whole
                # render, which runs on every track played by default
                # (THUMB_GEN=True).
                return await asyncio.to_thread(self._generate_sync, song, size, temp, output)
    ...
    def _generate_sync(self, song: Track, size, temp: str, output: str) -> str:
        ...
        image.convert("RGB").save(output, "JPEG", quality=getattr(config, "THUMB_QUALITY", 82), optimize=True, progressive=True)
        try:
            os.remove(temp)
        except Exception:
            pass
