...
class Queue:
    ...
    def check_item(self, chat_id: int, item_id: str) -> tuple[int, MediaItem | None]:
        ...
        for pos, track in enumerate(self.queues[chat_id]):
            if track.id == item_id:
                return pos, track
        return -1, None

    def force_play_new(self, chat_id: int, item: MediaItem) -> None:
        """Insert a brand-new item — NOT already anywhere in the queue
        — at the front, discarding whatever was playing before. Used
        by /play -f for a freshly-searched track. See force_add() for
        promoting an item that's already sitting in the queue; using
        this method for that case instead is exactly what caused the
        duplicate-track corruption bug (an id-based lookup can't tell
        two identical queued songs apart, and the old rotate-based
        removal math didn't actually remove the right one)."""
        self.remove_current(chat_id)
        self.queues[chat_id].appendleft(item)

    def force_add(self, chat_id: int, position: int) -> MediaItem | None:
        """Promote the item at 0-based `position` (already in the
        queue) to be the new currently-playing item, discarding
        whatever was playing before. Operates purely by position, not
        by track ID — so two identical songs queued at different
        positions can never be confused with each other. Returns the
        promoted item, or None if position is invalid or already 0
        (the current track — nothing to do)."""
    ...
    def set_current(self, chat_id: int, item: MediaItem) -> None:
        """Set the currently playing item without disturbing the rest of the queue.

        Playback is started after a track has been queued.  The voice layer then
        records the exact Media/Track object as position 0 so /queue, skip,
        autoplay and player-card updates all operate on the same object.
        """
        q = self.queues[chat_id]
        if q:
            q[0] = item
    ...
    def remove_at(self, chat_id: int, position: int) -> MediaItem | None:
        """Remove the queued item at a 1-based position (position 1 is
        always the currently playing track and can never be removed
        this way — /skip is what ends that one). Returns the removed
        item, or None if the position is out of range."""
        index = position - 1
        q = self.queues[chat_id]
    ...
    def move(self, chat_id: int, from_pos: int, to_pos: int) -> MediaItem | None:
        """Move the item at 1-based from_pos to 1-based to_pos. Position
        1 (currently playing) can't be moved or moved onto — same
        reasoning as remove_at. Returns the moved item, or None if
        either position is invalid."""
        q = self.queues[chat_id]
        n = len(q)
        from_i, to_i = from_pos - 1, to_pos - 1
